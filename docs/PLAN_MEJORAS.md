...
| M-04 | Alta | **Endurecer networking de los Key Vaults** (2026-07-11): `kv-goethe-mapas-dev|prod` se crearon con acceso desde **todas las redes públicas** (el portal lo advierte y Joaquín pidió documentarlo para corregirlo SÍ O SÍ). Mitiga mientras tanto: RBAC exige identidad de Azure AD para leer secretos aunque la red sea pública. **Corrección en el Hito 1**, cuando existan las Container Apps: en cada vault → Networking → **Allow public access from specific virtual networks and IP addresses** → (a) habilitar **"Allow trusted Microsoft services to bypass this firewall"**, (b) agregar las IPs fijas del colegio (181.30.18.114-118, para administrar por portal) y (c) la salida (egress) de las Container Apps si hiciera falta. Alternativa más fuerte (private endpoints + deshabilitar acceso público) queda descartada por costo salvo que se pida. | **Pendiente — se ejecuta en Hito 1** |
